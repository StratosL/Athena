...
| `init` | Initial project setup | `init: bootstrap Artemis project with Phase 1 foundation` |
...
3. **Include context future agents need:**
   ...
   - "Using VSA (Vertical Slice Architecture) pattern"
...
- **NEVER add `Co-Authored-By` trailers** to commit messages. The user is the sole author.

## Artemis Project Conventions

### Scope Examples for This Project
- `backend` - Backend infrastructure changes
- `frontend` - Frontend infrastructure changes
- `api` - API endpoint changes
- `ui` - UI component changes
- `tasks` - Task management feature (Phase 2)
- `daily-plan` - Daily planning feature (Phase 3)
- `pomodoro` - Pomodoro timer feature (Phase 4)
- `analytics` - Analytics feature (Phase 5)
- `docker` - Docker/containerization changes
- `deps` - Dependency updates
- `docs` - Documentation changes

### Architecture Notes for Future Agents
- **VSA Pattern**: Features in `backend/app/features/`, core in `backend/app/core/`
- **Brand System**: Stratos Louvaris design system with indigo/orange/cyan
