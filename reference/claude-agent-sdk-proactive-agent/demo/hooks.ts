/**
 * DEMO: Hooks - Intercepting agent behavior at lifecycle points
 *
 * - PreToolUse: fires BEFORE a tool runs (can block or modify inputs)
 * - PostToolUse: fires AFTER a tool runs (for logging/metrics)
 * - Stop: fires when session ends (cleanup/notifications)
 * - Includes safety hook that blocks Bash commands containing "rm"
 * - Use "matcher" to target specific tools
 *
 * Run: bun run demo:hooks
 */
