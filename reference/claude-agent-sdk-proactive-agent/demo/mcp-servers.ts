/**
 * DEMO: MCP Servers - Custom tools via Model Context Protocol
 *
 * - Creates 3 fake tools: get_weather, query_database, send_notification
 * - Uses Zod schemas to define and validate tool inputs
 * - Tools bundled into MCP server with createSdkMcpServer()
 * - Tool naming: mcp__<server-name>__<tool-name>
 * - IMPORTANT: Must use streaming input mode for custom MCP tools
 *
 * Run: bun run demo:mcp
 */
