/**
 * DEMO: Permissions - Controlling agent autonomy levels
 *
 * - bypassPermissions: full autonomy, no prompts
 * - acceptEdits: auto-approves file operations only
 * - plan: read-only mode, can't execute changes
 * - Hook-based blocking: bypassPermissions + PreToolUse hook to selectively block tools
 * - Hooks are the way to do granular control in headless/autonomous scenarios
 *
 * Run: bun run demo:permissions
 */
