/**
 * DEMO: Subagents - Spawning specialized child agents
 *
 * - Defines 3 subagents: researcher (web search), code-analyzer (reads code), writer (creates reports)
 * - Each subagent has its own model (Haiku for simple, Sonnet for complex), tools, and prompt
 * - Main agent uses the Task tool to delegate work to subagents
 * - Subagents have isolated context (separate conversation)
 *
 * Run: bun run demo:subagents
 */
